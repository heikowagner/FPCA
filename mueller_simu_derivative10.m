...
p = setDerOptions('selection_k', 2,'screePlot',0, 'corrPlot',0,...
            'designPlot',0,'numBins',10, 'nder',0:2, 'verbose','off')     
...
hXm1i=[hXm1i,  mean( (hX2lm-realD).^2) ];
j=j+1
...
Ph1ia=[Ph1ia , mse_ord(pcdReal(:,1),V2a(:,1),33) ]
Ph2ia=[Ph2ia , mse_ord(pcdReal(:,2),V2a(:,2),33) ]
Ph1ib=[Ph1ib , mse_ord(pc0Real(:,1),V2b(:,1),33) ]
Ph2ib=[Ph2ib , mse_ord(pc0Real(:,2),V2b(:,2),33) ]
Ph1im=[Ph1im , mse_ord(pc0Real(:,1),phi22(:,1),33) ]
Ph2im=[Ph2im , mse_ord(pc0Real(:,2),phi22(:,2),33) ]
...
meanScores=[medianScoresa ,[T N mean(Scoresa) mean(Scoresb) ]' ]
medianScoresa=[medianScoresa ,[T N median(Scoresa) median(Scoresb)  ]' ]
...
dlmwrite('mean10.csv', [ meanM(1:2,:) ; round( meanM(3:5,:)*10,2) ]', 'precision','%.2f')
dlmwrite('varV10.csv', [ varV(1:2,:) ; round( varV(3:5,:)*10^3,2) ]', 'precision','%.2f')
dlmwrite('medM10.csv', [ medM(1:2,:) ; round( medM(3:5,:)*10^1,2) ]', 'precision','%.2f')
dlmwrite('iqrM10.csv', [ iqrM(1:2,:) ; round( iqrM(3:5,:)*10^1,2) ]', 'precision','%.2f')
dlmwrite('meanPhi10.csv', [ meanPhi(1:2,:) ; round( meanPhi(3:8,:)*10^1,2) ]', 'precision','%.2f')
dlmwrite('meedianPhi10.csv', [ medianPhi(1:2,:) ; round( medianPhi(3:8,:)*10^1,2) ]', 'precision','%.2f')
dlmwrite('eigenvalues10.csv', [ eigenvalues(1:2,:) ; round( eigenvalues(3:6,:)*10^10,2) ]', 'precision','%.2f')
dlmwrite('medianeigenvalues10.csv', [ Medeigenvalues(1:2,:) ; round( Medeigenvalues(3:6,:)*10^10,2) ]', 'precision','%.2f')
dlmwrite('meanScores10.csv', [ meanScores(1:2,:) ; round( meanScores(3:6,:)*10,2) ]', 'precision','%.2f')
dlmwrite('medianScores10.csv', [ medianScoresa(1:2,:) ; round( medianScoresa(3:6,:)*10,2) ]', 'precision','%.2f')

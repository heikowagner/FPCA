...
p = setDerOptions('selection_k', 2,'screePlot',0, 'corrPlot',0,...
            'designPlot',0,'numBins',30, 'nder',0:2, 'verbose','off')     
...
dlmwrite('mean.csv', [ meanM(1:2,:) ; round( meanM(3:5,:)*10,2) ]', 'precision','%.2f')
dlmwrite('varV.csv', [ varV(1:2,:) ; round( varV(3:5,:)*10^3,2) ]', 'precision','%.2f')
dlmwrite('medM.csv', [ medM(1:2,:) ; round( medM(3:5,:)*10^1,2) ]', 'precision','%.2f')
dlmwrite('iqrM.csv', [ iqrM(1:2,:) ; round( iqrM(3:5,:)*10^1,2) ]', 'precision','%.2f')
dlmwrite('meanPhi.csv', [ meanPhi(1:2,:) ; round( meanPhi(3:8,:)*10^1,2) ]', 'precision','%.2f')
dlmwrite('eigenvalues.csv', [ eigenvalues(1:2,:) ; round( eigenvalues(3:6,:)*10^10,2) ]', 'precision','%.2f')

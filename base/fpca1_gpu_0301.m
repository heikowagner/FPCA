...
function [hX2r, V2a, loadsa,Meansmo2b,Da,x] = fpca1(L,Fc,xminc,xmaxc,cgridx,Xc,Y,Ytrue,x,method,comp,sigma)
...
for i=1:N
   ...
   if(method==1)
       if comp=='gpu'
            [XmiS0m XmiS1m XmiS2m XmiS3m, W0, W1, W2, W3]=multiloc( gpuArray(cell2mat( Xc(i) )) , gpuArray(cell2mat(  Y(i) )),gpuArray(x) ,gpuArray(hm(i,:).^(-1/3)),1,'Epan'  ); %%Estimate loc poly 
           else 
            [XmiS0m XmiS1m XmiS2m XmiS3m, W0, W1, W2, W3]=multiloc( cell2mat( Xc(i) ) , cell2mat(  Y(i) ),x ,hm(i,:).^(-1/3),1,'Epan'  ); %%Estimate loc poly           
       end
...
W= [W, W0];
              ...
              if comp=='gpu' 
                   [XmiS0m XmiS1m XmiS2m XmiS3m, W0, W1, W2, W3]=multiloc( gpuArray(cell2mat( Xc(i) )) , gpuArray(cell2mat(  Y(i) )),gpuArray(x) ,gpuArray(hm(i,:).^(-1/10)),7,'Epan'  );
              else
                
                   [XmiS0m XmiS1m XmiS2m XmiS3m, W0, W1, W2, W3]=multiloc( cell2mat( Xc(i) ) , cell2mat(  Y(i) ),x ,hm(i,:).^(-1/10),7,'Epan'  );
              end
...
parfor i=1:N
    if comp=='gpu'
        ...
        [XmiS0a XmiS1a XmiS2a XmiS3a]=multiloc( gpuArray(cell2mat(  c5unil(i) )),gpuArray(cell2mat(  c2unil(i) )) , gpuArray(cell2mat(  c3unil(i) )),mye ,mxe ,[h1a(1)^(1/10), Cdp *h1a(2)^(1/10)] ,3,'Gauss' ); %%Estimate loc poly
        ...
        %[XmiS0a XmiS1a XmiS2a XmiS3a]=multiloc( cell2mat(  c5unil(i) ),cell2mat(  c2unil(i) ) ,cell2mat(  c3unil(i) ),my ,mx ,h1a, Cdp *h2a ,3,'Epan' ); %%Estimate loc poly
          [XmiS0a XmiS1a XmiS2a XmiS3a, W0, W1, W2, W3]=multiloc( cell2mat( Xc(i) ) , cell2mat(  Y(i) ),x ,[h1a(1)^(1/10), Cdp *h1a(2)^(1/10)],3,'Gauss'  );
...
%scatter3(cgrid(:,1),cgrid(:,2),hX2r(:,2))
